...
public class HeaderImpl implements Header {
    ...
    public static final String RESOURCE_TYPE = "sample/components/structure/header";
     ...
     */
    private List<HeaderItem> getItems(NavigationRoot navigationRoot, Page subtreeRoot) {
        List<HeaderItem> pages = new ArrayList<>();
        if (navigationRoot.structureDepth == -1 || getLevel(subtreeRoot) < navigationRoot.structureDepth) {
            ...
            while (it.hasNext()) {
                ...
                List<HeaderItem> children = getItems(navigationRoot, page);
                boolean isSelected = checkSelected(page);
                if (structureStart == 0) {
                    level = level + 1;
                }
                pages.add(new HeaderItemImpl(page, isSelected, request, level, children));
            }
        }
        return pages;
    }

    private List<HeaderItem> getNavigationTree(NavigationRoot navigationRoot) {
        List<HeaderItem> itemTree = new ArrayList<>();
        ...
        if (structureStart == 0) {
            ...
            HeaderItemImpl root = new HeaderItemImpl(navigationRoot.page, isSelected, request, 0, itemTree);
    ...
    private boolean currentPageIsRedirectTarget(Page page) {
        ...
        if (contentResource != null) {
            ...
            String redirectTarget = valueMap.get("cq:redirectTarget", String.class);
